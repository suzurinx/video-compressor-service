...
try:
    ...
    print("仕様書通りの32バイトヘッダーを受信しました。")

    # 2. 先頭の4バイト（!I）からファイルサイズを解剖
    file_size, = struct.unpack("!I", header_bytes[:4])
    print(f"【解剖結果】これから届くファイルサイズ: {file_size}バイト")

    # 3. 動画データを受信して保存（名前は固定になります）
